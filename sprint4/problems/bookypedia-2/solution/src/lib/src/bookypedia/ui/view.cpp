...
std::string ClearSpaces(std::string str) {
    ...
    auto SpaceCheck = [](const auto& ch) {
        return std::isspace(static_cast<unsigned char>(ch));
    };
    auto begin = std::find_if(str.begin(), str.end(), SpaceCheck);
    while (begin != str.end()) {
        const auto end = std::find_if_not(begin, str.end(), SpaceCheck);
        ++begin;
        str.erase(begin, end);
        begin = std::find_if(begin, str.end(), SpaceCheck);
    }
...
std::optional<detail::AuthorInfo> View::ReadAuthor() const {
    ...
    if (!author) {
        ...
        return std::optional<detail::AuthorInfo>({domain::AuthorId::New().ToString(), std::move(name)});
    }
    ...
    return std::optional<detail::AuthorInfo>({author->GetId().ToString(), std::move(name)});
}
...
std::string View::ReadNewAuthorName() const {
...
bool View::EditAuthor(std::istream& cmd_input) const {
    try {
        ...
        use_cases_.EditAuthorName(domain::AuthorId::FromString(author_id), ReadNewAuthorName());
    } catch (const std::exception& e) {
        std::cerr << e.what() << std::endl;
...
bool View::DeleteAuthor(std::istream& cmd_input) const {
    try {
        ...
        if (auto id = ReadAuthor(cmd_input)) {
            author_id = *id;
        } else if (auto author = SelectAuthor()) {
            author_id = author->id;
        } else {
            return true;
        }
...
bool View::AddBook(std::istream& cmd_input) const {
    try {
        if (auto params = GetBookParams(cmd_input)) {
            use_cases_.AddBook({domain::AuthorId::FromString(std::move(params->author_id)), params->author_name},
                               std::move(params->title), params->publication_year, std::move(params->tags));
        }
    } catch (const std::exception& e) {
        std::cerr << e.what() << std::endl;
...
std::optional<std::vector<std::string>> View::ReadTags(std::string_view message) const {
    ...
    std::transform(tags.begin(), tags.end(), tags.begin(), [](const auto& tag) { return util::ClearSpaces(tag); });
...
std::vector<detail::BookInfo> View::GetBooks() const {
    ...
    for (const auto& book : books) {
        ...
        result.emplace_back(book.GetId().ToString(), book.GetTitle(), book.GetPublicationYear(), author.GetName(),
                            book.GetTags());
    }
...
std::vector<detail::BookInfo> View::GetAuthorBooks(const std::string& id) const {
...
std::string View::ReadBookTitle(std::string default_title) const {
    output_ << "Enter new title or empty line to use the current one (" << default_title << "):" << std::endl;
...
int View::ReadBookPublicationYear(int default_year) const {
    output_ << "Enter publication year or empty line to use the current one (" << default_year << "):" << std::endl;
...
std::vector<std::string> View::ReadBookTags(const std::vector<std::string>& default_tags) const {
    auto tags = ReadTags("Enter tags (current tags: " + boost::algorithm::join(default_tags, ", ") + "):");
