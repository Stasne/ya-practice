...
constexpr static auto BagCapacity{"bagCapacity"};
...
Map tag_invoke(boost::json::value_to_tag<Map>, const boost::json::value& jv) {
    ...
    std::optional<uint32_t> bagCapacity;
    if (obj.contains(Fields::BagCapacity))
        bagCapacity = static_cast<uint32_t>(obj.at(Fields::BagCapacity).as_int64());
    Map map(id, name, dogSpeed, bagCapacity);
