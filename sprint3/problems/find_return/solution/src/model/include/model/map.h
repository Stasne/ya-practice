...
public:
    ...
    Map(Id id, std::string name, std::optional<double> dogSpeed = std::nullopt,
        std::optional<uint32_t> bagCapacity = std::nullopt) noexcept
        : id_(std::move(id)), name_(std::move(name)), speed_(dogSpeed), bagCapacity_(bagCapacity) {}

    const Id&               GetId() const noexcept { return id_; }
    const std::string&      GetName() const noexcept { return name_; }
    std::optional<double>   GetMapSpeed() const { return speed_; }
    std::optional<uint32_t> GetBagCapacity() const { return bagCapacity_; }
...
private:
    ...
    std::optional<uint32_t> bagCapacity_;
    Roads                   roads_;
