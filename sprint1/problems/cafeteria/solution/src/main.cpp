...
void PrintHotDogResult(const Result<HotDog>& result, Clock::duration order_duration)
{
    ...
    if (result.HasValue())
    {
        ...
        os << "Hot dog #" << hot_dog.GetId() << ": bread(" << hot_dog.GetBread().GetId()
           << ") bake time: " << as_seconds(hot_dog.GetBread().GetBakingDuration()) << "s, sausage("
           << hot_dog.GetSausage().GetId() << ") bake time: " << as_seconds(hot_dog.GetSausage().GetCookDuration())
           << "s" << std::endl;
    }
    else
    {
...
std::vector<HotDog> PrepareHotDogs(int num_orders, unsigned num_threads)
{
    ...
    for (int i = 0; i < num_orders; ++i)
    {
        ...
        // нескольких потоках
        net::dispatch(io,
                      [&cafeteria, &hotdogs, &mut, i, start_time, &start, num_waiting_threads]
                      {
                          ...
                          cafeteria.OrderHotDog(
                              [&hotdogs, &mut, start_time](Result<HotDog> result)
                              {
...
int main()
{
    ...
    // Ожидаемое время приготовления 20 хот-догов на 4 рабочих потоках: от 6.5 до 7.5 секунд
    //
